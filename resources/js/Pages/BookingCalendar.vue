...
// Add these new refs at the top
const dragPreviewElement = ref(null);
const currentDragCell = ref(null);
...
function openEditModal(booking) {
    // Don't open edit modal if we just finished resizing
...
function handleDragStart(event, booking, date) {
  ...
  // Get cell width (assuming all cells are equal width)
  const cell = event.currentTarget.closest('td');
  cellWidth.value = cell ? cell.offsetWidth : 140; // default fallback
  
  // Create drag preview
  if (!dragPreviewElement.value) {
    dragPreviewElement.value = document.createElement('div');
    dragPreviewElement.value.className = 'drag-preview';
    document.body.appendChild(dragPreviewElement.value);
  }
  
  // Style the preview to match the booking
  const bookingElement = event.currentTarget.querySelector(`[data-booking-id="${booking.id}"]`);
  if (bookingElement) {
    const rect = bookingElement.getBoundingClientRect();
    dragPreviewElement.value.style.width = `${rect.width}px`;
    dragPreviewElement.value.style.height = `${rect.height}px`;
    dragPreviewElement.value.style.backgroundColor = window.getComputedStyle(bookingElement).backgroundColor;
  }
  
  currentDragCell.value = cell;
}
...
function handleDragOver(event) {
  ...
  if (!isDragging.value || !draggedBooking.value) return;
  
  // Update drag offset from the start position
  ...
  if (dragPreviewElement.value) {
    ...
    if (cell) {
      ...
      // Calculate days offset based on pixel movement
      const daysOffset = Math.round(dragOffsetX.value / cellWidth.value);
      
      // Position the preview - this should match the actual drop calculation
      dragPreviewElement.value.style.left = `${cellRect.left}px`;
      dragPreviewElement.value.style.top = `${cellRect.top}px`;
      dragPreviewElement.value.style.display = 'block';
      
      // Optional: Update the preview element with information about the move
      dragPreviewElement.value.textContent = `${draggedBooking.value.customer_name} (${daysOffset > 0 ? '+' : ''}${daysOffset} days)`;
    }
  }
}
...
function handleDrop(event, targetRoom, targetDate) {
  ...
  // Remove preview
  if (dragPreviewElement.value) {
    dragPreviewElement.value.style.display = 'none';
  }
  
  const bookingId = draggedBooking.value.id;
  const originalCheckIn = parseISO(draggedBooking.value.check_in);
  const originalCheckOut = parseISO(draggedBooking.value.check_out);
  const duration = differenceInDays(originalCheckOut, originalCheckIn);
  
  // Calculate days offset based on total drag distance
  const daysOffset = Math.round(dragOffsetX.value / cellWidth.value);
  
  // Get the correct target date by adding the daysOffset to the target cell's date
  // This ensures alignment with the preview
  const adjustedTargetDate = format(addDays(targetDate, 0), 'yyyy-MM-dd');
  ...
  // Handle date changes
  if (targetRoom === draggedBooking.value.room) {
    // If same room, apply the day offset to calculate new dates
    updatedBooking.check_in = format(addDays(originalCheckIn, daysOffset), 'yyyy-MM-dd');
    updatedBooking.check_out = format(addDays(originalCheckOut, daysOffset), 'yyyy-MM-dd');
  } else {
    // For room changes without horizontal movement, keep original dates
    updatedBooking.check_in = format(originalCheckIn, 'yyyy-MM-dd');
    updatedBooking.check_out = format(originalCheckOut, 'yyyy-MM-dd');
  }
...
function stopResize() {
  ...
  if (resizingBooking.value && resizingOriginalBooking.value) {
   ...
   // Set the flag to prevent edit modal from opening immediately
   justFinishedResizing.value = true;
  
  // Reset the flag after a short delay
  setTimeout(() => {
    justFinishedResizing.value = false;
  }, 300); // 300ms should be enough to prevent the click
